...
fn vector_and_point_operator_overloading_integrity() {
    ...
    assert!(Vector3::new(1.50, 3.75, 4.0) == (v2 / 2.0));
    // Neg Vector3
    assert_eq!(-v1, Vector3::new(-2.0, -3.5, -4.0));
    // Sub two Point3
    assert_eq!(p1 - p2, Vector3::new(-0.5, -3.5, -3.5));
    // Div Point3 by scalar
    assert_eq!((1.25, 1.75, 2.25), (p1.x / 2.0, p1.y / 2.0, p1.z / 2.0));
    // Mul Point3 by scalar
    assert!(p3 == Point3::new(2.55553, 7.88887, 9.34342));
    // Neg Point3
    assert_eq!((-p1.x, -p1.y, -p1.z), (-2.5, -3.5, -4.5));
